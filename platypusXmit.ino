...
void setup() {
  ...
  pinMode(15, INPUT_PULLUP); //comment out for debug
...
void noteFire() {
  ...
  for (int i = 0 ; i < 12; i ++) {
    if (cap0.touched() & (1 << i)) {
      if (noteOn0[i] == 0 ) {
        ...
        if (scale > 0) {
          Velocity = ((cap0.filteredData(i)) / 6);
          Velocity = map(Velocity, 127, scaler[scale], 0, velCap);
          Velocity = constrain(Velocity, 0, velCap);
        }
    ...
    if (noteOn0[i] == 1) {
      if (chPress == true) {
        int pres = ((cap0.filteredData(i)) / 6);
        pres = map(pres, 127, scaler[scale], 0, 127);
        pres = constrain(pres, 0, 127);
  ...
  for (int i = 0 ; i < 12; i ++) {
    if (cap1.touched() & (1 << i)) {
      if (noteOn1[i] == 0 ) {
        ...
        if ( scale > 0 ) {
          Velocity = ((cap1.filteredData(i)) / 6);
          Velocity = map(Velocity, 127, scaler[scale], 0, velCap);
          Velocity = constrain(Velocity, 0, velCap);
        }
        packet.velocity = Velocity;
    ...
    if (noteOn1[i] == 1) {
      if (chPress == true) {
        int pres = ((cap1.filteredData(i)) / 6);
        pres = map(pres, 127, scaler[scale], 0, 127);
        pres = constrain(pres, 0, 127);
        packet.pressure = pres;
        packet.statusByte = 0xD0 | channel;
  ...
  for (int i = 0 ; i < 12; i ++) {
    if (cap2.touched() & (1 << i)) {
      if (noteOn2[i] == 0 ) {
        ...
        if ( scale > 0) {
          Velocity = ((cap2.filteredData(i)) / 6);
          Velocity = map(Velocity, 127, scaler[scale], 0, velCap);
          Velocity = constrain(Velocity, 0, velCap);
        }
        packet.velocity = Velocity;
    ...
    if (noteOn2[i] == 1) {
      if (chPress == true) {
        int pres = ((cap2.filteredData(i)) / 6);
        pres = map(pres, 127, scaler[scale], 0, 127);
        pres = constrain(pres, 0, 127);
        packet.pressure = pres;
        packet.statusByte = 0xD0 | channel;
  ...
  if (cap3.touched() & (1 << 0)) {
    if (noteOn3 == 0 ) {
      ...
      if (scale > 0) {
        Velocity = ((cap3.filteredData(0)) / 6);
        Velocity = map(Velocity, 127, scaler[scale], 0, velCap);
        Velocity = constrain(Velocity, 0, velCap);
      }
      packet.velocity = Velocity;
  ...
  if (noteOn3 == 1) {
    if (chPress == true) {
      int pres = ((cap3.filteredData(0)) / 6);
      pres = map(pres, 127, scaler[scale], 0, 127);
      pres = constrain(pres, 0, 127);
      packet.pressure = pres;
      packet.statusByte = 0xD0 | channel;
